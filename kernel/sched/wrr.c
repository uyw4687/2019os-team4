...
static void enqueue_task_wrr(struct rq *rq, struct task_struct *p, int flags)
{
    ...
    if(!((wrr_load_balance_running && p->wrr.is_lb_task) || (wrr_round_robin_running && p->wrr.is_rr_task) || (wrr_set_sched_running && p->wrr.is_ss_task))) {
...
int select_task_rq_wrr(struct task_struct *p, int prev_cpu, int sd_flag, int wake_flags)
{
    ...
    for_each_online_cpu(cpu) {
        ...
        if(cpu == 3 || !cpumask_test_cpu(cpu, &p->cpus_allowed))
            break;
...
extern void __migrate_swap_task(struct task_struct *p, int cpu);
...
#if DEBUG
        pr_err("\n\nload balance task %d, busiest cpu %d weight %d, freest cpu %d weight %d, task weight %d\n\n", movable_highest_weight_task->pid, busiest->cpu, max_weight, freest->cpu, min_weight, movable_highest_weight_task->wrr.weight);
#endif
    }
